...
def edit_Data(df):
    ...
    response = AgGrid(df_template, editable=True, fit_columns_on_grid_load=False, conversion_errors='coerce',gridOptions=gridOptions, enable_enterprise_modules=True, allow_unsafe_jscode=True,height=grid_height)
    st.write(response)
    st.button("Upload to Database")
    if st.button:
        df_template = response['data']
        save_data(df_template, 'mms_master.sqlite', tbl_name)
